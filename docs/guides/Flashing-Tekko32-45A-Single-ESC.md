...
7. Open the bootloader project
    ...
    - Check that your selected target is something with `32k bootloader`, not `64k bootloader`. The picture below is outdated and shows a target `Target 1`.
...
8.  Changing the signal input pin in the Bootloader Source from the `PA2` to the `PB4`:
    ...
    - Find the lines with the define macros for `USE_PA2`, `USE_PB4`. Uncomment the one you want; comment-out the other. (2)
...
9.  Building the bootloader binary
   ...
   - When the build runs, you will see console logs in the lower left section.
     
     Carefully check for Warnings and Errors.

     It may give warnings about some obsolete compiler flags but no errors.

   At this point we have a ESC specific bootloader binary available.
...
11. Checking if the debugger finds your powered target mcu.
    - In the tree view, right-click on the target name usually "32k_Bootloader", click "Options for ..."
    ...
    - If everything is connected correctly, you will see your target under `SW Device`. If you see no target connected, check your wiring.

      ![Fig. 14](https://user-images.githubusercontent.com/125764759/219981601-a36ed44e-04f7-42a5-801c-1368bd60798f.png)

    Remember this dialog. We will return here a couple of times.
...
12. Invalidating the flash protection.
    ...
    - Still in the Settings menu go to *Flash Download*-Tab
    - In the table *Programming Algorithm* (2), use *Remove* (3) and *Add* (4) buttons to end up with only `AT32F421 Flash user system data` (1)
    ...
    - On the main screen select *Flash* -> *Download* or press F8 or click the "Download" button in the toolbar.

      It will throw an error message which can be ignored. 
...
13. Flashing the actual Bootloader
    ...
    - Return to the main screen and download like in the previous step.
      ...
      It should now flash and print some log output which must contain something like "partial verify done". That is your all-ok indicator.

      It will also throw a warning that some address range has been skipped, `AT32F421 Flash user system data` in particular. This is intended.
