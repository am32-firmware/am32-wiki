...
Some Settings are named differently between the web and desktop client, where names differ significantly, aka names are for desktop client.
...
|3d Mode (AKA: Bi-Directional)|Used for cars, normally crawlers. Throttle range is divided between forward and reverse where centre stick position is neutral||
...
|Sinusoidal Startup|Slow mode startup. Better torque at low speeds, but generates more heat. |Good for Crawlers|
|30ms Telemetry|Enables a serial telemetry output on a 30ms interval using the Kiss telemetry protocol||
|Use Hall Sensors|Not currently implemented due to lack of hall sensor capable targets||
